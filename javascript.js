...
while (uniqueNumbers.size < count) {
  uniqueNumbers.add(Math.floor(Math.random() * (max - 0 + 1)) + 0);
}
...
let klausimai="";
let atsakymoPasirinkimas="";
let teisingasAtsakymas=0;
let teisinguAtsakymuKiekis=0;

const randomKlausimai=generateRandomNumbers(questions1.length);
console.log(`${randomKlausimai} klausimu masyvas`)

for (i=0; i<questions1.length; i++){
    const randomAtsakymuPasirinkimas=generateRandomNumbers(questions1[randomKlausimai[i]].answers.length);
    console.log(`${randomAtsakymuPasirinkimas}-pasirinkimu masyvas  ${randomKlausimai[i]}-klausimas`);
    for(j=0; j<questions1[randomKlausimai[i]].answers.length; j++){
        console.log(`j-${j}`);
        atsakymoPasirinkimas=(j+1).toString();
        klausimai=klausimai+atsakymoPasirinkimas+": "+questions1[randomKlausimai[i]].answers[randomAtsakymuPasirinkimas[j]].text+"\n";

        if(questions1[randomKlausimai[i]].answers[randomAtsakymuPasirinkimas[j]].isCorrect){
            teisingasAtsakymas=j+1;
        }
        
    }
    let klausimoAtsakymas=prompt(`${questions1[randomKlausimai[i]].question}\n${klausimai}`);
    if(klausimoAtsakymas==teisingasAtsakymas){
        alert(`Atsakymas ${klausimoAtsakymas} yra teisingas`)
        teisinguAtsakymuKiekis++;
        klausimai="";
    }
    else if(isNaN(klausimoAtsakymas) || klausimoAtsakymas>questions1[randomKlausimai[i]].answers.length || klausimoAtsakymas<0 || klausimoAtsakymas==""){
        alert(`Atsakymas ivestas neteisingai, pakartokite ivedima`);
        i--;
        klausimai="";
        continue;
    }
    else{
        alert(`Atsakymas ${klausimoAtsakymas} yra neteisingas \n teisingas atsakymas yra ${teisingasAtsakymas}:${questions1[randomKlausimai[i]].answers[teisingasAtsakymas-1].text}`)
    }
    teisingasAtsakymas=0;
    klausimai="";
}
    alert(`Is ${questions1.length} buvo atsakyta ${teisinguAtsakymuKiekis} klausimai \n jusu pazymys yra:${(teisinguAtsakymuKiekis/(questions1.length))*10}`)